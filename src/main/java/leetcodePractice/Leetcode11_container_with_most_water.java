...
public class Leetcode11_container_with_most_water {
    public static int maxArea(int[] height) {
        ...
        int min = Math.min(height[head], height[end]);

        if (height.length > 1) {
            maxArea = min * (end - head);
        }

        for (int i = head, j = end; i < j; ) {
            while(height[i] <= min && i < j && i < height.length) {
                i++;
            }

            while(height[j] <= min && i < j && j >= 0) {
                j--;
            }
    ...
    public static int maxArea_brute_force(int[] height) {
        ...
        for (int i = 1; i < height.length; i++) {
            ...
            for (int j = 0; j < i; j++) {
                int area = Math.min(h, height[j]) * (i - j);
                if (area > maxArea) {
                    maxArea = area;
                }
            }
